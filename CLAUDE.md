...
- **`generate` should return the solution alongside the puzzle**: Currently `generate()` returns only `Puzzle`, discarding the solution it started from. Callers (e.g., WASM bindings) must re-solve to recover it. Change the return type to include the `Solution` (e.g., return `(Puzzle, Solution)`) to avoid the redundant solve.
